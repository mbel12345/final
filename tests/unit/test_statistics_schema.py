...
def test_schema_average_result_response_valid():

    # Test creating a valid AverageResultResponse schema
    ...
    response = AverageResultResponse(**data)
    assert response.type == data['type']
    assert response.average == data['average']


def test_schema_average_result_response_type_is_none():

    # Test AverageResultResponse passes if type is None
...
def test_schema_average_result_response_missing_average():

    # Test AverageResultResponse fails if 'average' is missing
    ...
    with pytest.raises(ValidationError, match='required'):
        AverageResultResponse(**data)


def test_schema_average_result_response_invalid_type():

    # Test AverageResultResponse fails if 'type' is invalid
